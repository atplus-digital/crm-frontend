...
function setReferenceValue<T>(reference: Ref<T> | undefined, value: T | null) {
	if (!reference) {
		return;
	}
...
export function KanbanBoardCardTextarea({
	...
	const handleReferenceChange = useCallback(
		(node: HTMLTextAreaElement | null) => {
			...
			setReferenceValue(externalReference, node);
		},
		[externalReference],
	);
...
export function KanbanBoardCardButton({
	...
	const button = (
		// biome-ignore lint/a11y/useSemanticElements: The card container is already a <button>, so nested buttons are invalid HTML.
		<div
			className={cn(
