...
│   │   │   └── nocobase-client.ts # NocoBaseApiClient base class (inalterado)
│   │   ├── io/
│   │   │   ├── atomic-writer.ts    # Atomic file writer (inalterado)
...
│   │   │   ├── lifecycle.ts        # runStandardPipeline() — prepare → lock → backup → stages → assert → cleanup
...
export async function runStandardPipeline<C, R>(options: {
  ...
  lockWorkspace: () => void;
  backupOutputs: (ctx: C) => void;
  stages: AsyncPipelineStage<C["pipelineContext"]>[];
  assertResult: (ctx: C) => R;
  cleanupBackups: (ctx: C) => Promise<void>;
}): Promise<R>;
```

Fluxo: `prepare → lock workspace → backup outputs → run stages → assert → cleanup`

Stages usam `task.output = "mensagem"` para output — Listr2 renderiza automaticamente com spinners e cores.
...
```typescript
// lib/cli/tasks.ts
export function createPreparationTasks<C>(opts: {
  createContext: (override: Partial<...>, task: ListrTaskWrapper) => C;
  lockWorkspace: () => void;
  backupOutputs: (ctx: C) => void;
}): GeneratorTask<C>[]

export function createFinalizationTasks<C, R>(opts: {
  assertResult: (ctx: C) => R;
  cleanupBackups: (ctx: C) => Promise<void>;
}): GeneratorTask<C>[]
...
- **Atomic writer** (`lib/io/atomic-writer.ts`)
- **Reports** (`lib/reports.ts`)
- **HTTP client** (`lib/http/http-client.ts`)
- **NocoBase API client** base class (`lib/http/nocobase-client.ts`)
