...
/**
 * Verifica se o workspace está configurado para bloquear a edição dos
 * arquivos gerados.
 */
export function isWorkspaceLocked(): boolean {
	try {
		...
		const outputPattern = toReadonlyPattern(config.outputDir);
		return outputPattern in readonlyInclude;
	} catch (error) {
		logger.warn(
			`Não foi possível verificar as configurações do workspace: ${error}`,
		);
		return false;
	}
}
...
/**
 * Bloqueia o acesso de escrita aos arquivos gerados adicionando as
 * configurações apropriadas ao arquivo .vscode/settings.json.
 */
function lockWorkspace(): void {
	try {
		...
		const outputPattern = toReadonlyPattern(config.outputDir);
		if (!(outputPattern in readonlyInclude)) {
			readonlyInclude[outputPattern] = true;
		}
