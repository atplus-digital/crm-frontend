import type { NocoBaseCredentials } from "@scripts/generators/src/pipelines/generate-types/@types/nocobase";
import { NocoBaseDataSourceClient } from "@scripts/generators/src/pipelines/generate-types/utils/client";
...
describe("NocoBaseDataSourceClient", () => {
	...
	describe("fetchCollectionFields", () => {
		it("should return sorted fields alphabetically by name", async () => {
			...
			expect(fields[0].name).toBe("alpha_field");
			expect(fields[1].name).toBe("beta_field");
			expect(fields[2].name).toBe("zebra_field");
		});
	});
	...
	describe("fetchCollectionFields (with fallback enabled)", () => {
		it("should fall back to listSample on 404 response", async () => {
			...
			const { fields } = await client.fetchCollectionFields("nonexistent");
		...
		it("should return empty array when sample has no records", async () => {
			...
			const { fields } = await client.fetchCollectionFields("empty_collection");
