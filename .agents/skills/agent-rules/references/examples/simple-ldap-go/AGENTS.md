...
# AGENTS.md — simple-ldap-go

**Project:** simple-ldap-go — Simple LDAP client library in Go  
**Stack:** Go 1.25, LDAP SDK  
...
```
cmd/             → example CLI usage
pkg/ldap/        → LDAP client library
...
| ------------- | --------------------- | ------------------------------------- |
| LDAP Client   | `pkg/ldap/client.go`  | Connection management, query methods  |
| Example Usage | `cmd/example/main.go` | Client initialization, error handling |
...
- Expose LDAP credentials in error messages
