...
1. **Animation package mismatch** (`tailwindcss-animate` vs `tw-animate-css`)
...
5. **In Vite projects, prefer `@tailwindcss/vite`** (PostCSS is still supported in Tailwind v4 for non-Vite pipelines)
...
4. **Rely only on `tailwind.config.ts` for shadcn semantic tokens**

   ```typescript
   /* WRONG for shadcn v4 token workflow */
   export default {
     theme: {
       extend: {
...
5. **Overuse `@apply` for regular component styling**
...
### ❌ Avoid These Legacy Choices

```bash
# Legacy animation plugin used in many v3 setups:
bun add tailwindcss-animate  # ❌ Deprecated in shadcn v4 flow
# or: npm install tailwindcss-animate
```

**For shadcn v4 projects**, migrate to:
